  ...
  switch (er.code) {
      ...
      if (process.platform !== "win32" && (isCachePath || isCacheDest)) {
        ...
        summary.push([
          ...
          [
            ...
            `  sudo chown -R ${process.getuid()}:${process.getgid()} "${npm.config.get(
              "cache"
            )}"`,
          ].join("\n"),
        ]);
    ...
    case "EBADPLATFORM": {
      ...
      for (const key of checkedKeys) {
        const padding =
          key.length === longestKey ? 1 : 1 + (longestKey - key.length);

        // padding + 1 because 'actual' is longer than 'valid'
        detailEntry.push(
          `Valid ${key}:${" ".repeat(padding + 1)}${expected[key]}`
        );
