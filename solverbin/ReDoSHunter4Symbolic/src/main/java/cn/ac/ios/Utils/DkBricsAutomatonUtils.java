...
//                        stringBuilder.append("[" + allLeafs.get(i).getData() + "]");
    ...
    public static Pair<String, SmtNode> getExampleByDkBricsAutomaton(List<String> regexList1) throws InterruptedException {
    ...
    public static String getExampleByDkBricsAutomaton(List<String> regexList1, List<String> regexList2) throws InterruptedException {
        return getExampleByDkBricsAutomaton(regexList1, regexList2, true, -1, -1).getKey();
    }
    ...
    public static String getExampleByDkBricsAutomaton(List<String> regexList1, List<String> regexList2, boolean accepted) throws InterruptedException {
        return getExampleByDkBricsAutomaton(regexList1, regexList2, accepted, -1, -1).getKey();
    }
    ...
    public static String getExampleByDkBricsAutomaton(List<String> regexList1, List<String> regexList2, boolean accepted, int minLength) throws InterruptedException {
        return getExampleByDkBricsAutomaton(regexList1, regexList2, accepted, minLength, -1).getKey();
    }

    public static Pair<String, SmtNode> getExampleByDkBricsAutomaton(List<String> regexList1, List<String> regexList2, int minLength) throws InterruptedException {
...
//                System.out.println(data + " " + firstNum + " " + secondNum);
    ...
    // 获取前缀匹配串
    public static Pair<String, SmtNode> getPrefixString(TreeNode root, TreeNode left, boolean considerZeroWidthAssertions) throws InterruptedException {
        // 获取前缀 + 中缀正则
        ...
        if (considerZeroWidthAssertions) {  // 如果考虑零宽断言
            ...
            return new Pair<>(getExampleByDkBricsAutomaton2(translateRegexForLookAroundList, 0).getKey(), new SmtNode(translateRegexForLookAroundList));
        } else {    // 如果不考虑零宽断言
            // 删除零宽断言
            ...
            return new Pair<>(getExampleByDkBricsAutomaton2(regexList1, 0).getKey(), new SmtNode(regexList1));
        }
    }

    // 获取后缀不匹配串
    public static List<Pair<String, SmtNode>> getSuffixString(TreeNode root, TreeNode left, String infixString) throws InterruptedException {
        // 生成后缀的方式
        // r2 -> 非空的r2
        ...
        if (r3Tree.getData().equals("")) {
            ...
            Pair<String, SmtNode> example = getExampleByDkBricsAutomaton2(regexList1, 1 + infixString.length());
            List<Pair<String, SmtNode>> exampleList = new ArrayList<>();
            if (example.getKey() == null) {
                exampleList.add(new Pair<String, SmtNode>("", new SmtNode(Collections.singletonList(""))));
            } else {
                exampleList.add(new Pair<String, SmtNode>(example.getKey(), new SmtNode(regexList1)));
            }

            if (! transInfixString.equals("")) {
                for (int i = 0; i < exampleList.size(); i++) {
//                    String z = exampleList.get(i).replaceFirst("\\Q" + transInfixString + "\\E", "");
                    Pair<String, SmtNode> z = exampleList.get(i);
                    if (z.getKey().startsWith(infixString)) {
//                        z = z.substring(infixString.length());
                        z.setKey(z.getKey().substring(infixString.length()));
                    }
                    exampleList.set(i, z);
                }
            }
...
//            }
            ...
            List<Pair<String, SmtNode>> exampleList = new ArrayList<>();
            Pair<String, SmtNode> example1 = getExampleByDkBricsAutomaton2(regexList1, 1 + infixString.length());
            if (example1 == null) {
                exampleList.add(new Pair<>("", new SmtNode(Collections.singletonList(""))));
            } else {
                exampleList.add(new Pair<>(example1.getKey(), new SmtNode(regexList1)));
            }
...
//        return prefix != null ? prefix : "";
        ...
        List<String> translateRegexForLookAroundList = getTranslateRegexForAssertionsList(newTreeNode);
        return getExampleByDkBricsAutomaton2(translateRegexForLookAroundList, 0).getKey();
    }
...
//        System.out.println("restoreAssertPatternRegex = " + restoreAssertPatternRegex);
        String prefixAndInfixString = getExampleByDkBricsAutomaton(Collections.singletonList(restoreAssertPatternRegex)).getKey();
...
//        System.out.println("restoreAssertPatternRegex = " + restoreAssertPatternRegex);
        ...
        String suffix = getExampleByDkBricsAutomaton(regex1List, null, 0).getKey();
...
//        String suffix = getExampleByDkBricsAutomaton(regex1List, null, 0);
        String suffix = getExampleByDkBricsAutomaton2(regex1List, 0).getKey();
